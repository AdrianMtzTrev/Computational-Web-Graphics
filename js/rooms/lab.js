...
export class LabRoom {
  ...
  async _buildSciFiWalls(scene) {
    ...
    const setup = (obj, color) => {
      obj.traverse(c => { if (c.isMesh) { c.castShadow = true; c.receiveShadow = true; if (c.material) { const ms = Array.isArray(c.material) ? c.material : [c.material]; ms.forEach(m => { if (m.color && !m.map) m.color.setHex(color || 0x99aabb); }); } } });
    };
  ...
  _buildFurniture(scene) {
    ...
    const equipment = (x, z, color, emissiveColor) => {
      ...
      const bodyMat = new THREE.MeshStandardMaterial({ color, emissive: emissiveColor || color, emissiveIntensity: 0.05, metalness: 0.3, roughness: 0.5 });
